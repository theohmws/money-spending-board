...
CI (`.github/workflows/CI.yml`) runs on Node 20 for: `build-prod`, commitlint (on PRs), `lint`, `check-types`, `test`, and `test-storybook:ci`. There's no CI e2e step — `e2e:headless`/Percy are dropped from CI (vanity-engineering-review: they only ever exercised the stale Pages-Router boilerplate Cypress specs, not the actual board product) and the build matrix is single-Node (dropped the second Node version — no bug had ever surfaced there); both scripts still exist in `package.json` for optional local/manual use.
