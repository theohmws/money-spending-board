...
Defines Google OAuth as a second sign-in method alongside email/password on the auth screen, reusing `useAuthSession`'s existing Supabase client, session-resolution path, and loading/error state rather than introducing a parallel OAuth-specific code path.
