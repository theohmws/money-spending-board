...
Defines the app's installability and offline-shell behavior: the web app manifest's required fields/icons, the service worker's precache/offline-fallback/cross-origin-passthrough behavior, and registration being scoped to production + capable browsers only.

## Requirements

### Requirement: App exposes a web app manifest declaring it as installable
The app SHALL expose a web app manifest (via `app/manifest.ts`, statically generated) declaring `name`, `short_name`, `start_url`, `display: standalone`, `background_color`, `theme_color`, and an `icons` array containing at least 192×192 and 512×512 entries plus one `purpose: maskable` entry. `app/layout.tsx`'s `metadata` SHALL reference the manifest and include `themeColor` (via the `viewport` export) and `appleWebApp` fields so both the W3C manifest path (Android/desktop Chrome) and Apple's non-manifest install metadata (iOS Safari) are covered.
