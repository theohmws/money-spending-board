...
Defines the composition architecture for the spending board's client-side state: a composition-root hook (`useSpendingBoard`) that assembles independently testable, domain-sliced hooks, with defined ownership boundaries and cross-slice derived-view responsibilities.
