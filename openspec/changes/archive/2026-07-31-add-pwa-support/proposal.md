...
- **App icons**: new 192×192, 512×512, and maskable-512 PNG icons for the manifest's `icons` array, derived from a user-supplied illustrated app-icon graphic (a rounded-square dashboard scene in the board's navy/green palette). `favicon.ico`, `favicon-16x16.png`, `favicon-32x32.png`, and `apple-touch-icon.png` — previously the untouched Next.js Boilerplate placeholder — are regenerated from the same artwork so the browser tab/bookmark icon matches the PWA install icon.
