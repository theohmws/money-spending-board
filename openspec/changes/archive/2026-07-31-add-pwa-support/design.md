...
**4. New icons are derived from a user-supplied illustrated icon graphic, not the existing 180×180 `apple-touch-icon.png`.**
Upscaling a 180px source to 512px would visibly soften/blur, and the provided artwork (a 1254×1254 rounded-square illustration of the board's dashboard — budget-ratio donut, spending trend, wallet/coins) is both higher-resolution and a deliberate icon design. The source PNG's outer corners were flat-black (a flattened/letterboxed background around the rounded-square shape, not transparency); those corners are keyed out via a corner-seeded flood fill (tolerant of the artwork's anti-aliased edge) rather than a naive color threshold, so interior dark tones (the navy clipboard border, wallet, calculator) aren't accidentally punched through. The result is cropped to its opaque bounding box and re-padded to a square before being resized down to 192×192 and 512×512.
The maskable variant can't reuse the transparent-cornered square as-is (maskable icons must be fully opaque edge-to-edge per the maskable-icon spec) — it composites the artwork at ~72% scale, centered, onto an opaque backdrop sampled from the artwork's own navy border color, keeping the illustration inside the ~80% safe-zone circle so Android's adaptive-icon masking doesn't clip it.
Alternative considered: crop just the small circular piggy-bank badge from the artwork's top-left corner as a simpler, more legible icon at small sizes. Rejected per explicit user preference for the full illustrated scene.
...
- **[Risk] A cached service worker can serve a stale app shell after a deploy if the cache-name constant isn't bumped.** → Mitigation: `scripts/set-sw-cache-version.js` rewrites `sw.js`'s `CACHE_NAME` constant to `msb-shell-v<package.json version>` as part of `npm run build`, so it tracks the package version automatically (which `semantic-release` bumps on every release to `main`) instead of relying on a developer remembering to hand-edit the constant; `sw.js`'s `activate` handler deletes any cache not matching the current name. Kept as a small standalone script rather than a bundler plugin, matching the "small hand-written file" decision in #2.
