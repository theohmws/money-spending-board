...
- [ ] 9.3 **Not done.** No browser available in this session to manually walk the migration end to end (sign in with pre-existing `localStorage` data, confirm migration, reload, confirm no further `localStorage` reads). Worth doing once this ships to a real environment — see the `verify` skill.
