...
- Separately, every imported row shows its own permanent "imported" badge (based on `source`), regardless of review status — so a row's origin stays visible even after it's been cleaned up, distinct from the transient needs-review badge.
- Neither is a filled pill: needs-review is a left-edge accent stripe on the row (carrying an `aria-label` too, since color alone isn't legible to everyone), and the source badge is a bare colored text label next to the subtitle. Both colors are user-configurable (not hardcoded), persisted on Supabase the same way `categoryMeta`'s colors are persisted to `localStorage` today — as a single JSON settings blob, just DB-backed so it follows the user across devices.
...
- New migration(s) (`supabase/migrations/`): `transactions` gains `source text` and `needs_review boolean not null default false`; new `spending_board.import_category_rules` table; new `spending_board.board_settings` table (one row per user, `badge_colors jsonb`) — all RLS scoped by user, following the existing `transactions` policy pattern.
- New hook: `useCreditCardImport` (or similar), composed into `useSpendingBoard.ts` alongside the other domain hooks.
- New components: an import entry point (button in `BoardHeader` or `TransactionList`), a password-prompt step, a preview modal with an editable table, a small settings surface for the category-rule mapping, and a small settings surface for the two badge colors (all three settings surfaces likely folded into the existing `CategorySettingsModal` area rather than three wholly new modals — TBD in design).
